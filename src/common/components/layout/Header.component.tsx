...
const MainHeader: React.FC <FooterProps> = ({headerLinks, headerHrefs, showNavDropdown,ssLogoImg,handleDropdownClick,notifyPageMissing}) => {
  return (
    ...
    <header className="flex h-header w-full fixed top-0 left-0 z-50 py-4 items-center justify-between bg-navy-blue-bg 
                           2xl:px-16 xl:px-16 lg:px-16 s-laptop:px-16 tablet:px-8 mobile:px-8"  style={{ boxShadow: '0 0px 15px #000' }}> 
          <div className="flex h-full w-auto">
            ...
            <div className="flex-col w-auto px-4
                            2xl:flex xl:flex lg:flex s-laptop:flex tablet:flex mobile:hidden">
          ...
          {!showNavDropdown && <HiMenu className="2xl:hidden xl:hidden lg:flex s-laptop:flex tablet:flex mobile:flex" style={{ 'cursor': 'pointer'}} color="white" size={"2.5rem"} onClick={() => { handleDropdownClick(true)}} /> }
          {!!showNavDropdown && <HiOutlineX className="2xl:hidden xl:hidden lg:flex s-laptop:flex tablet:flex mobile:flex" style={{ 'cursor': 'pointer'}} color="white" size={"2.5rem"}  onClick={() => { handleDropdownClick(false)}} /> }
        ...
        { !!showNavDropdown && <div className="flex flex-col h-auto w-full fixed top-h-header left-0 z-50 py-4 items-center justify-between bg-white
                           2xl:px-8 xl:px-8 lg:px-8 s-laptop:px-8 tablet:px-4 mobile:px-4 2xl:hidden xl:hidden lg:flex m:flex tablet:flex mobile:flex"  style={{ boxShadow: '0 15px 25px #222' }}> 
            { headerLinks.map((element, index) => {
              return (
